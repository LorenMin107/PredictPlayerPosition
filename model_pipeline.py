...
def evaluate_model(pipeline: Pipeline, X_test: pd.DataFrame, y_test: np.ndarray, 
    ...
    if y_prob is not None:
        ...
        # Convert y_test to one-hot encoding for ROC-AUC calculation
        y_test_onehot = np.zeros((len(y_test), n_classes))
        for i in range(n_classes):
            y_test_onehot[:, i] = (y_test == i).astype(int)
...
def plot_classification_report(evaluation_results: Dict[str, Any], save_path: Optional[str] = None):
    """
    Create and display a heatmap visualization of the classification report.
...
def plot_roc_curves(evaluation_results: Dict[str, Any], save_path: Optional[str] = None):
    """
    Create and display ROC curves for multi-class classification.
    ...
    if evaluation_results['roc_auc'] is None:
        print("ROC-AUC metrics not available. Skipping ROC curve visualization.")
        return

    # Extract data from evaluation results
    roc_auc_dict = evaluation_results['roc_auc']
    model_name = evaluation_results['model_name']
...
def run_full_pipeline(data_path: str, artifacts_dir: str = 'artifacts') -> Dict[str, Any]:
    ...
    plot_classification_report(evaluation_results, f"{artifacts_dir}/classification_report_heatmap.png")

    # 13. Plot ROC curves (if applicable)
    if evaluation_results['roc_auc'] is not None:
        plot_roc_curves(evaluation_results, f"{artifacts_dir}/roc_curves.png")
